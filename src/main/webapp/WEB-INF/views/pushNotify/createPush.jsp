...
<ul class="breadcrumb">
    ...
    <li><a href="<%= request.getContextPath()%>/backend/create-push">新しいPUSHを作成</a></li>
</ul>
<script>var ctx = "${pageContext.request.contextPath}";</script>
<form:form action="add-push" commandName="pushNotify" method="post" id="formCreatePush">
            ...
            <div id="pushConfirmModal" class="modal fade modal-lg bg-primary-modal " role="document" style="margin: 30px auto; background: none;">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button id="btnConfirmCreatePush" type="submit" class="btn btn-primary" data-confirm="Ok">OK</button>
...
<script>
    ...
    function validatePushRequest() {
        ...
        var pushRequest = {}
        pushRequest["timerDateStr"] = $('#inputTimerDate').val();
        ...
        $.ajax({
            ...
            url : ctx + "/backend/validate-push",
            data : JSON.stringify(pushRequest),
            timeout : 100000,
            success: function(response) {
                if(response.error == 1){
                    document.getElementById("msgTimerDate").innerHTML= response.errorMsg;
                }else{
                    ...
                    if($('#inputIOSNumber').val() >= 0 && iosChecked){
                        pushIos = $('#inputIOSNumber').val();
                    }
                    if($('#inputAndroidNumber').val() >= 0 && androidChecked){
                        pushAndroi = $('#inputAndroidNumber').val();
                    }
    ...
    $('#btnConfirmCreatePush').click(function () {
