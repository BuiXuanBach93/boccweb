...
<form:form action="update-banner" id="bannerEditForm" modelAttribute="banner" method="POST" enctype="multipart/form-data">
        ...
        <div class="box-body" style="margin-top: 10px; padding: 10px">
            <div class="row">
                ...
                <div class="col-md-2">
                    ...
                    <form:input type="hidden" id="inputBannerId" path="bannerId"/>
                </div>
            </div>
            <div class="row" style="margin-top: 20px;">
                ...
                <div class="col-md-4">
                    <div>
                        <img id="bannerImage" src='${banner.imageUrl}' alt="" height="60" width="428"
                             onerror="this.onerror=null;this.src='${pageContext.request.contextPath}/resources/images/noimage.png';"/>
                        <form:input type="hidden" id="inputIsChangeImage" path="isChangeImage"/>
                    </div>
            ...
            <div class="row" style="margin-top: 20px;">
                ...
                <div action="" class="col-md-6" id="group-filter-type" style="padding-top: 5px;">
                    <c:choose>
                        <c:when test="${banner.desTypeWebUrl == true}">
                        ...
                        <c:otherwise>
                            <input id="inputDesTypeWebUrl" type="radio" name="desType" value="0" onclick="handleClickDesTypeRadio(this);">  <label for="inputDesTypeWebUrl">WebView</label>
                        </c:otherwise>
                    </c:choose>
                    <c:choose>
                        <c:when test="${banner.desTypeCategoryId == true}">
                            <input id="inputDesTypeCategoryId" type="radio" name="desType" value="1" style="margin-left: 20px;" checked onclick="handleClickDesTypeRadio(this);"><label for="inputDesTypeCategoryId">カテゴリID</label>
                        </c:when>
                        <c:otherwise>
                    ...
                    <c:choose>
                        <c:when test="${banner.desTypePostId == true}">
                            <input id="inputDesTypePostId" type="radio" name="desType" value="2" style="margin-left: 20px;" checked onclick="handleClickDesTypeRadio(this);"><label for="inputDesTypePostId">投稿ID</label>
                        </c:when>
                        <c:otherwise>
                    ...
                    <c:choose>
                        <c:when test="${banner.desTypeKeyword == true}">
                            <input id="inputDesTypeKeyword" type="radio" name="desType" value="3" style="margin-left: 20px;" checked onclick="handleClickDesTypeRadio(this);"><label for="inputDesTypeKeyword">キーワード検索結果</label>
                        </c:when>
                        <c:otherwise>
            ...
            <div class="row" style="margin-top: 20px; display: none; " id="groupRadioUrlTypePanel">
                ...
                <div action="" class="col-md-6" id="group-url-type" style="padding-top: 5px;">
                    <c:choose>
                        ...
                        <c:otherwise>
                            <input id="inputUrlFillType" type="radio" name="urlType" value="0" onclick="handleClickUrlTypeRadio(this);" > <label for="inputUrlFillType">URL入力</label>
                            <input id="inputUrlBuildType" type="radio" name="urlType" value="1" onclick="handleClickUrlTypeRadio(this);" checked style="margin-left: 20px;"><label for="inputUrlBuildType">キャンペーンページ作成</label>
                        </c:otherwise>
                    </c:choose>
                </div>
            </div>
            <div class="row" style="margin-top: 20px;">
                ...
                <div class="col-md-4">
                    <form:input id="inputDesText" type="text" path="destinationText" class="form-control"/>
                </div>
            </div>
            <div id="bannerPageBuildPanel" class="row" style="margin-top: 20px; display: none">
                ...
                <div class="col-md-10" style="padding-top: 10px; padding-bottom: 10px;">
                    ...
                    <div class="row">
                        ...
                        <%--</div>--%>
                        <%--<div class="col-md-1">--%>
                            <%--<div id="divColorTitle"  style="background: ${banner.titleColor}; width: 34px; height: 34px;">--%>
                            <%--</div>--%>
                        <%--</div>--%>
                    </div>
                    <div class="row" style="margin-top: 10px;">
                        ...
                        <div  class="col-md-1">
                            <div id="divColorBackground"  style="background: ${banner.backgroundColor}; width: 34px; height: 34px;">
                            </div>
                        </div>
                    </div>
                    <div class="row" style="margin-top: 20px;">
                        ...
                        <div class="col-md-10">
                            <div>
                                <img id="pageImage" src='${banner.imagePageUrl}' alt="" width="300"
                                     onerror="this.onerror=null;this.src='${pageContext.request.contextPath}/resources/images/noimage.png';"/>
                                <c:choose>
                                    <c:when test="${banner.imagePageUrl == null}">
                                        <input type="hidden" id="inputSelectedPageImage" value="0"/>
                                    </c:when>
                                    <c:otherwise>
                                        <input type="hidden" id="inputSelectedPageImage" value="1"/>
                                    </c:otherwise>
                                </c:choose>
                            </div>
                            <div class="fileUpload btn btn-primary" style="margin-left: 0px;">
...
<script>
    ...
    var urlTypeValue = $('input[name=urlType]:checked').val();
    if(urlTypeValue == 1 && desTypeValue == 0){
        ...
        if (input.files && input.files[0]) {
            ...
            img.onload = function () {
                if(this.width != 715 && this.height != 100){
                    document.getElementById("errMsg").innerHTML= "正しい画像形式をアップロードしてください。";
                    return;
                }else {
                    ...
                    // update change Image flag
                    document.getElementById("inputIsChangeImage").value = 1;
                }
            };
...
//        }
    ...
    $('#btnSaveBanner').click(function() {
        ...
        bannerRequest["bannerId"] = $('#inputBannerId').val();
        var desTypeValue = $('input[name=desType]:checked').val();
        if(desTypeValue == 0){
